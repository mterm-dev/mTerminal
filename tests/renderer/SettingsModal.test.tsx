...
describe("SettingsModal — AI section", () => {
  ...
  it.each([
    ...
    ["openai"],
    ["ollama"],
  ])("default provider radio %s emits update", (p) => {
    const props = makeProps({
      settings: { ...DEFAULT_SETTINGS, aiEnabled: true, aiDefaultProvider: "anthropic" },
    });
    render(<SettingsModal {...props} />);
    gotoSection(/^AI$/);
    const btn = screen.getByRole("button", { name: new RegExp(`^${p}$`) });
    fireEvent.click(btn);
    expect(props.update).toHaveBeenCalledWith("aiDefaultProvider", p);
  });

  it("anthropic model input reflects value and emits update", () => {
    const props = makeProps({
      settings: { ...DEFAULT_SETTINGS, aiEnabled: true, aiAnthropicModel: "claude-3" },
    });
    render(<SettingsModal {...props} />);
    gotoSection(/^AI$/);
    const input = screen.getByDisplayValue("claude-3") as HTMLInputElement;
    fireEvent.change(input, { target: { value: "claude-4" } });
    expect(props.update).toHaveBeenCalledWith("aiAnthropicModel", "claude-4");
  });
